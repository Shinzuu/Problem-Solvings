...
int main()
{
    ...
    while(t--){
        ...
        if(n==1)
            ...
            minDiff = v[1]-v[0];
            f(i,1,n){
                minDiff = gcd( abs(v[i]-v[i-1]) , minDiff);
                //cout << i<<" is : " << minDiff << endl;
            } 
            ll targ =1;
            ll o = n-1;
            while(o > 0){
                
                if(v[o]- minDiff == v[o-1])
                {
                    o--;
                    //duplicate found
                }
            ...
            if(targ == -1)
                ...
                targ = v[n-1];
                ans+= (v[n-1] - (v[n-1]-o*minDiff) ) /minDiff;

            } 
            //cout << targ << endl; 
            f(i,0,n){
                ans +=  ((targ-v[i])/minDiff); 
                //cout << "d : " << (((v[n-1]+minDiff)-v[i])/minDiff) << endl;              
            }
            cout <<  ans << endl;
        }
